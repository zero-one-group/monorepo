  ...
  ({ className, type, showCopyButton, showExternalCopyButton, onCopy, ...props }, ref) => {
    ...
    return (
      <div className={styles.wrapper()}>
        ...
        {showCopyButton && !type && (
          <CopyButton onCopy={handleCopy} copyButtonClass={styles.copyButton()} />
        )}
      </div>
    )
  }
...
const PasswordToggle = ({ showPassword, onToggle, toggleButtonClass }: PasswordToggleProps) => (
  <TooltipProvider>
    <Tooltip delayDuration={150}>
      <TooltipTrigger asChild>
        <button type="button" onClick={onToggle} className={toggleButtonClass}>
          {showPassword ? (
...
const CopyButton = ({ onCopy, copyButtonClass }: CopyButtonProps) => (
  <TooltipProvider>
    <Tooltip>
      <TooltipTrigger asChild>
        <button type="button" onClick={onCopy} className={copyButtonClass}>
...
interface ExternalCopyButtonProps {
  onCopy: () => void
}

const ExternalCopyButton = ({ onCopy }: ExternalCopyButtonProps) => (
  <TooltipProvider>
    <Tooltip>
      <TooltipTrigger asChild>
        <Button
