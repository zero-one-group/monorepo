...
TEMPLATE_SOURCE_NAME="react-app"
TEMPLATE_TARGET_NAME="react-app"

echo "Building React SPA project templates..."
...
# Replace string "3000" with "{{ port_number }}" in any files that contain it except "template.yml"
grep -rl "3000" "$TARGET_PATH" | grep -v "template.yml" | while read -r file; do
    replace_string "$file" "3000" "{{ port_number }}"
done
