...
# Replace string "1337" with "{{ port_number }}" in any files that contain it except "template.yml"
grep -rl "1337" "$TARGET_PATH" | grep -v "template.yml" | while read -r file; do
    replace_string "$file" "1337" "{{ port_number }}"
done
