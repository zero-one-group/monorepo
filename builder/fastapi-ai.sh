...
TEMPLATE_SOURCE_NAME="fastapi-ai"
TEMPLATE_TARGET_NAME="fastapi-ai"

echo "Building FastAPI-AI project templates..."
...
# Replace string "8080" with "{{ port_number }}" in any files that contain it except "template.yml"
grep -rl "8080" "$TARGET_PATH" | grep -v "template.yml" | while read -r file; do
    replace_string "$file" "8080" "{{ port_number }}"
done
...
# Rename all files *.py to *.raw.py inside the target directory
find "$TARGET_PATH" -type f -name "*.py" | while read -r file; do
    mv "$file" "${file%.py}.raw.py"
done
