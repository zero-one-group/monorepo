...
# Replace string "3200" with "{{ port_number }}" in any files that contain it except "template.yml"
grep -rl "3200" "$TARGET_PATH" | grep -v "template.yml" | while read -r file; do
    replace_string "$file" "3200" "{{ port_number }}"
done
