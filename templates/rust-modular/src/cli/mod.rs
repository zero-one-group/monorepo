...
//!
//! ```text
//! {{ package_name | kebab_case }} [serve]                      # default subcommand
//! {{ package_name | kebab_case }} migrate run                  # apply pending migrations
//! {{ package_name | kebab_case }} migrate create <name>        # scaffold a new migration file
//! {{ package_name | kebab_case }} migrate down                 # NOT SUPPORTED (no revert scripts)
//! {{ package_name | kebab_case }} migrate reset                # drop public schema + re-apply
//! {{ package_name | kebab_case }} seed                         # load dev seed data
//! {{ package_name | kebab_case }} generate-config [--output]   # emit .env.example
//! ```
//!
...
/// {{ package_name | kebab_case }} CLI entrypoint.
#[derive(Parser, Debug)]
#[command(
    name = "{{ package_name | kebab_case }}",
    version,
    about = "Rust port of the Zero One Group {{ package_name | kebab_case }} service (Phase D)",
    long_about = None,
)]
pub struct Cli {
    #[command(subcommand)]
...
fn migrate_create(name: &str) -> Result<()> {
    ...
    let path = PathBuf::from("apps/{{ package_name | kebab_case }}/migrations").join(&filename);
    ...
    let header = format!(
        "-- Migration: {trimmed}\n\
         ...
         -- `{{ package_name | kebab_case }} migrate reset` to roll back in dev.\n\n"
    );
...
fn render_env_example(cfg: &Config) -> String {
    ...
    let _ = writeln!(out, "# {{ package_name | kebab_case }} environment configuration");
