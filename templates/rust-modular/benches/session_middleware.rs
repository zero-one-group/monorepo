...
use {{ package_name | snake_case }}::AppState;
use {{ package_name | snake_case }}::apputils::PasswordHasher;
use {{ package_name | snake_case }}::build_router;
use {{ package_name | snake_case }}::config::Config;
...
async fn setup_bench() -> BenchCtx {
    let container = Postgres::default()
        .with_db_name("{{ package_name | snake_case }}_bench")
        .with_user("postgres")
    ...
    let database_url =
        format!("postgres://postgres:postgres@127.0.0.1:{host_port}/{{ package_name | snake_case }}_bench");
