...
test('user can sign in then sign out (state flows through context)', async () => {
  ...
  await waitFor(() => {
    expect(screen.getByTestId('dashboard-welcome')).toHaveTextContent('Welcome, test@example.com')
  })
...
test('invalid credentials show a helpful error', async () => {
  ...
  await waitFor(() => {
    expect(screen.getByTestId('auth-error')).toHaveTextContent('Enter a valid email.')
  })
})
