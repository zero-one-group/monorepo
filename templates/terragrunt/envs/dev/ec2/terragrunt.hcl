...
locals {
  ...
  variables = yamldecode(file("${get_parent_terragrunt_dir()}/variables/ec2/variables.yaml"))
...
inputs = {
  ...
  instances = {
    ...
    nginx-1 = {
      ami_instance       = local.variables.ubuntu_noble_ami
      instance_type      = local.variables.instance_types["nginx"]
      subnet_id          = dependency.vpc.outputs.subnet_public_all[0]
      ...
      enable_eip        = true
      eip_allocation_id = dependency.elastic_ip.outputs.eip_nginx_id
      s3_bucket_arns = [
        "arn:aws:s3:::lgtm-monitoring"
      ]
      ssm_parameter_paths = [
